  ...
  int Receive_Serial_package()
  {
    ...
    while ((GetTickCount() < endwait))
    {
      if (rxbyte_waiting(serial))
      {
        ...
        if (i >= 1544)
        {            // Check MAX_BUFFER size received
...
std::string FindDeviceBySerial(const std::string& serial_id_substr) {
  ...
  int test(int argc, char **argv) // , std::string serial_id_substr)
  {
    ...
    // Adding a new way to assign the ACM port to avoid hardcoding, this is not necessary
    ...
    for (i = 0; i < argc; i++)
    {
      ...
      else if (argv[i][0] == 'x')
      {
        ...
        if (num_bytes > 768)
        { // ERROR parsing data. Too many data inputs
          print_help();
...
public:
  RawBoson(int argc, char **argv) // , std::string serial_id_substr)
  {
    // Recieve serial id substring from somewhere added by Anton
    test(argc, argv);
  }
};

RawBoson send_command(int count...)
{
  va_list args;
  va_start(args, count);
  char *argv[count + 1];
  ...
  return RawBoson(sizeof(argv) / sizeof(char *), argv);
}

void shutter(const std::string serial_port)
{
  send_command(2, "c50007", ("-p" + serial_port).c_str());
}

std::string xstr(int num)
{
...
{
  send_command(6, "c50014", "x0", "x0", "x0", xstr(gain_mode).c_str(), ("-p" + serial_port).c_str());
}

int get_gain_mode(const std::string serial_port)
{
  RawBoson rb = send_command(2, "c50015", ("-p" + serial_port).c_str());
  return rb.BosonData[3];
}

// 0 manual, 1 auto, 2 external, 3 shutter test
void set_ffc_mode(int ffc_mode, const std::string serial_port)
{
  send_command(6, "c50012", "x0", "x0", "x0", xstr(ffc_mode).c_str(), ("-p" + serial_port).c_str());
}

int get_ffc_mode(const std::string serial_port)
{
  RawBoson rb = send_command(2, "c50013", ("-p" + serial_port).c_str());
  return rb.BosonData[3];
}

// 0 no ffc performed, 1 ffc imminent, 2 ffc in progress, 3 ffc complete
int get_ffc_status(const std::string serial_port)
{
  RawBoson rb = send_command(4, "c50054", ("-p" + serial_port).c_str());
  return rb.BosonData[3];
}
...
{
  send_command(6, "c50098", "x0", "x0", "x0", xstr(sync_mode).c_str(), ("-p" + serial_port).c_str());
}

int get_sync_mode(const std::string serial_port)
{
  RawBoson rb = send_command(2, "c50099", ("-p" + serial_port).c_str());
  return rb.BosonData[3];
}
