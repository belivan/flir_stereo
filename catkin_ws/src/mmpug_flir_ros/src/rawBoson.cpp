...
RawBoson send_command_device(int device, int count...){
  va_list args;
  va_start(args, count);
  char*argv[count+1];
  argv[0] = "./rawBoson";
  ...
  return RawBoson(sizeof(argv)/sizeof(char*), argv, device);
}

void shutter(){
  send_command(1, "c50007");
}
...
void set_gain_mode(int gain_mode){
  send_command(5, "c50014", "x0", "x0", "x0", xstr(gain_mode).c_str());
}

int get_gain_mode(){
  RawBoson rb = send_command(1, "c50015");
  return rb.BosonData[3];
}

// 0 manual, 1 auto, 2 external, 3 shutter test
void set_ffc_mode(int ffc_mode){
  send_command(5, "c50012", "x0", "x0", "x0", xstr(ffc_mode).c_str());
}

int get_ffc_mode(){
  RawBoson rb = send_command(1, "c50013");
  return rb.BosonData[3];
}

// 0 no ffc performed, 1 ffc imminent, 2 ffc in progress, 3 ffc complete
int get_ffc_status(){
  RawBoson rb = send_command(1, "c50054");
  return rb.BosonData[3];
}

// 0 disable, 1 master, 2 slave
void set_sync_mode(int sync_mode, int device){
  send_command_device(device, 5, "c50098", "x0", "x0", "x0", xstr(sync_mode).c_str());
}

int get_sync_mode(){
  RawBoson rb = send_command(1, "c50099");
  return rb.BosonData[3];
}
